...
module.exports = function (program) {
    ...
    let groups = new ResourceGroups('','');
    //    let exts = new Extensions(program.credentials, program.subscription.id);
    let returnObj = {};
    let config = new Config();

    let name = 'azworkshops_basicAD_04190145459730';
            returnObj['Resource Group'] = name;
    ...
    return new Promise(() => { Promise.resolve(); })
        ...
        .then(() => { return exts.addPowershellExtension('AddMachineToAd', returnObj['Resource Group'], 'utility', program.location.name, ["https://raw.githubusercontent.com/AzureWorkshops/cli/master/scripts/add-machine-to-ad.ps1"], 'add-machine-to-ad.ps1 -Domain "' + returnObj['Domain Name'] + '" -DomainAdmin "' + returnObj['Domain Admin'] + '"', 'Adding \'utility\' VM to \'' + returnObj['Domain Name'] + '\' domain...'); })
        .then(() => {
